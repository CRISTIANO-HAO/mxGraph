	...
	var clickHandler = mxUtils.bind(this, function(evt)
	{
		...
		if(label.getAttribute('name') == 'firstLevelLabel') {
			for(var i = 0; i < document.getElementsByName('firstLevelLabel').length; i++) {
				document.getElementsByName('firstLevelLabel')[i].style.backgroundColor = '#ccc';
 ...
 * @constructor
 */
EditStylePanel = function(format, editorUi, container)
{
	BaseFormatPanel.call(this, format, editorUi, container);
	this.init();
};

mxUtils.extend(EditStylePanel, BaseFormatPanel);

EditStylePanel.prototype.init = function(){
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	var containsLabel = this.format.getSelectionState().containsLabel;
	...
	label2.style.backgroundColor = '#ccc';
	label3.style.backgroundColor = '#ccc';
	...
	else
	{
		...
		var stylePanel = div.cloneNode(false);
		stylePanel.setAttribute('name','secondLevelPanel');
		stylePanel.style.display = '';
		new StyleFormatPanel(this.format, ui, stylePanel);
		//this.panels.push(new StyleFormatPanel(this, ui, stylePanel));
		this.container.appendChild(stylePanel);

		clickLabel(this.format, label, stylePanel);
	}
	...
	textPanel.setAttribute('name','secondLevelPanel');
	new TextFormatPanel(this.format, ui, textPanel);
	this.container.appendChild(textPanel);

	clickLabel(this.format, label2, textPanel);
	...
	arrangePanel.setAttribute('name','secondLevelPanel');
	arrangePanel.style.display = 'none';
	new ArrangePanel(this.format, ui, arrangePanel)
	//this.panels.push(new ArrangePanel(this, ui, arrangePanel));
	this.container.appendChild(arrangePanel);

	clickLabel(this.format, label3, arrangePanel);
...
console.log(this.format.getSelectionState());
	...
	if(this.format.getSelectionState().style.shape.indexOf("机组") > -1) { // 机组
		panels = getAttributePanel("unit");
	}
	else if(this.format.getSelectionState().style.shape.indexOf("用户") > -1) { // 用户
		panels = getAttributePanel("user");
	}
	else if(this.format.getSelectionState().style.shape.indexOf("line") > -1) { // 线路
		panels = getAttributePanel("line");
	}
	else if(this.format.getSelectionState().style.shape.indexOf("节点") > -1) { // 线路
		panels = getAttributePanel("node");
	}
	...
	for(var i = 0; i < panels.length; i++) {
		...
		var panel = this.createAttrPanel(panels[i].fields);
 ...
 * @param attrs
 */
EditAttributePanel.prototype.createAttrPanel = function(attrs) {
	...
	console.log(cellId,modelBean[cellId]);
	if(!modelBean[cellId]) {
	...
	for(var i = 0; i < attrs.length; i++) {
		...
		span.innerHTML = attrs[i].fieldName;
		fieldDiv.appendChild(span);

		var value = "";
		if(modelBean[cellId][attrs[i].fieldId]!=null) {
			value = modelBean[cellId][attrs[i].fieldId];
		}
		else {
			modelBean[cellId][attrs[i].fieldId] = "";
		}

		// 属性编辑框/选择框
		if(attrs[i].fieldType) {
			var inputOrSelect;
			if(attrs[i].fieldType == 'input') {
				inputOrSelect = document.createElement('input');
			}
			else if(attrs[i].fieldType == 'select'){
				inputOrSelect = document.createElement('select');
				if(attrs[i].selectOptions) {
					for(var j=0;j<attrs[i].selectOptions.length;j++){
						inputOrSelect.add(new Option(attrs[i].selectOptions[j].text,attrs[i].selectOptions[j].value));
					}
				}
			}

			inputOrSelect.setAttribute('name',attrs[i].fieldId);
			inputOrSelect.style.width = '40%';
			inputOrSelect.style.border= '1px solid #ccc';
			inputOrSelect.style.borderRadius = '3px';
			inputOrSelect.value = value;
			inputOrSelect.onchange  = function(event){
				editor.setModified(true);
				changeAttributeValue(ui,cellId,event);
			}
		...
		if(attrs[i].fieldId == 'lineType' && value == 'connectLine') {
			break;
		}
	}
