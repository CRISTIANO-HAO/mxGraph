...
EditAttributePanel.prototype.createAttrPanel = function(attrs) {
	...
	for(var i = 0; i < attrs.length; i++) {
		...
		if(attrs[i].fieldId == 'lineType') {
			if(!graph.getModel().getCell(cellId).lineType || graph.getModel().getCell(cellId).lineType == 'connectLine') {
				modelBean[cellId]['lineType'] = 'connectLine';
			}
			else {
				modelBean[cellId]['lineType'] = 'line';
			}
		}

		var value = "";
		if(modelBean[cellId][attrs[i].fieldId]) {
